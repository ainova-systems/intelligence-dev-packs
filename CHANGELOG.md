...
- **`dev-deliver` no longer claims to invoke the two owner-gated skills.** Its `C to D` and `D` boundary rows read "then `git-merge-pr`" and "then `git-create-release`", and Phase D called for the latter directly - all three naming the orchestrator as the actor. Both skills carry `disable-model-invocation: true`, and on a host that honours it the model cannot invoke them or replicate them; the first real run of the flow hit exactly that refusal. Nothing about the outcome changes, because the owner's invocation *is* the approval those gates were always waiting for - so the rows now say the step runs **on** the accept or the authorization and name no actor, which is true on a host that reserves those skills for the owner and on one that lets a run call them. `flow_approvals` gains the consequence it was missing: an upfront grant moves when the owner acts, never whether, and no grant given earlier types a command. `docs/enforcement.md` drops the same false claim - it had asserted that a flow skill reaches both inside a run, directly contradicting the bullet beneath it that says the flag holds on Claude Code.

### Changed

- **`docs/enforcement.md` states why the new entry point carries no owner-gate flag.** `git-merge-pr` and `git-create-release` keep `disable-model-invocation: true` because invoking either *is* the irreversible act; `dev-deliver` reaches both inside a run and deliberately carries no such flag, because invoking it starts an interview and because a model that cannot reach it improvises the chain instead. What holds the line inside a run is that neither gate can be inferred.
