...
- **The owner gate holds at the end of the flow, and `dev-deliver` names no actor for it.** `git-merge-pr` and `git-create-release` keep `disable-model-invocation: true` because invoking either *is* the irreversible act; `dev-deliver` deliberately carries no such flag, because invoking it starts an interview, and because a model that cannot reach it improvises the chain instead. It does not thereby reach the two flagged skills: on a host that honours the flag, the model can neither invoke them nor replicate them, so a run hands its last two steps to the owner and the owner's invocation *is* the approval. The outcome is identical either way, which is why the skill states that the merge and the release run **on** the accept and the authorization without saying who performs them - the one thing true on every host the pack renders to. `flow_approvals` carries the consequence: an upfront grant moves when the owner acts, never whether, and no grant given earlier types a command. `docs/enforcement.md` records the same reading.
