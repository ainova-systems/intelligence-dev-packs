...
- Skills: a folder in `skills/` whose name matches the `name:` frontmatter of its `SKILL.md`. Self-contained steps, a `Verify` section, a `Scope / hand-off` section, and a `Constraints` section for true invariants. Resolve project specifics via the profile resolution order (profile, then detection, then ask once and record). A skill whose side effect is irreversible and outward-facing (merge, release) gates the **act**, not its own invocation: the command goes in `templates/claude-settings.json` under `permissions.ask`, so the owner is prompted at that command with the skill's guards already satisfied. Blocking the invocation instead breaks any skill that orchestrates it and gates nothing the guards had not already checked.
- References are written in backticks, and the validator depends on it - it has no other way to tell a reference from prose. A profile key appears as `` `qa_env` `` or `` `qa_env: auto` ``, and as ``profile `qa_env` `` when naming the lookup; written bare, it reads to the checker as a key nothing uses and fails with a message that names the wrong problem. An artifact is named the same way (`` `git-verify-pr` ``), and every such name must resolve to a real rule, agent, skill or template - which is what turns a rename into a build failure instead of a dangling hand-off that reads like a working one.
