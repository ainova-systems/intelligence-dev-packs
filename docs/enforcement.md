...
- **A flow skill is not a second door around the gate.** `dev-deliver` reaches `git-merge-pr` and `git-create-release` inside a run, and it carries no `disable-model-invocation` of its own - deliberately, because invoking it starts an interview rather than an irreversible act, and because a model that cannot reach it improvises the chain instead, which is the failure the skill exists to fix. The flag stays where invocation and the act are the same thing. What holds the line inside a run is that neither gate can be inferred: the accept and release approvals are the owner's words in that run, and a run with nobody to ask ends at the gate and reports.
