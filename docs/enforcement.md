...
- **A flow skill is not a second door around the gate.** `dev-deliver` carries no `disable-model-invocation` of its own - deliberately, because invoking it starts an interview rather than an irreversible act, and because a model that cannot reach it improvises the chain instead, which is the failure the skill exists to fix. It does not thereby reach the two flagged skills: on Claude Code the flag refuses a Skill-tool invocation *and* refuses replicating the workflow by other means, so a run reaches its last two steps by handing them to the owner, whose invocation is the approval. That is the gate working rather than a gap, and it is why `dev-deliver` states those two steps without naming who performs them - the skills ship to hosts that honour the flag and to hosts that ignore it, and only the owner's explicit act is common to both.
