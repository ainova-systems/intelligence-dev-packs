...
- The docs chain is the spine: feature docs (behavior today) link to a spec (`requirements` / `plan` / `tasks` per change) link to code, found by convention not search. At execution end the spec folds back into updated feature docs, durable rules, and decision records (`spec-execute` Phase D, `spec-decision`, `spec-audit-docs`).
