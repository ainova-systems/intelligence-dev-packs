...
4. When an answer changes structure, adjust the affected **unticked** work steps and the coverage table in the same edit; ticked steps are history and stay. A change that alters a requirement is drift - route it back through the intake (`spec-pull` re-pull, or an update via `spec-create`).
