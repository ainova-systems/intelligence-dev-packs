...
1. Resolve the spec (argument, or the most recently created `proposed` spec). Read requirements, plan, and tasks end-to-end.
2. Pre-checks before recording approval: the spec is `proposed` (not already approved / in-progress / completed / cancelled); the plan cites concrete siblings; no open question is left that would block a cold executor. Surface anything that should block approval - the owner decides, this skill only records the decision.
...
- Authoring or changing the spec - `spec-create` (update-mode). Executing - `spec-execute` / `spec-execute-next`.

## CRITICAL

- Approval is the owner's call: the AI never sets `approved` on its own initiative (not inside `spec-create`, not autonomously) - only when the owner explicitly invokes this skill after reviewing.
