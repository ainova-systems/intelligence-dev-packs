...
**Depends on `core`**: the orchestrators invoke `core`'s git and verification skills (`git-finalize-pr`, `git-complete-pr`, `git-merge-pr`, `git-commit-push`, `dev-run-tests`). Install `core` alongside it.
