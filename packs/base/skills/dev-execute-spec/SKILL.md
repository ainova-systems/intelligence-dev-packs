...
5. **Commit at milestones** via `dev-commit-push` - one commit = one shippable-for-testing unit. Tick `tasks.md` boxes and update the plan in the same commit as the code that earns them.
...
2. Run `dev-finalize-pr` - CI to green plus every review comment handled.
...
- No spec yet - `dev-create-spec` first (the owner reviews it before this skill runs).
- Resuming a half-done spec - `dev-continue-spec`.
- Merging - `dev-merge-pr`, only after owner accept.
