...
- Replace a capability this host lacks before escalating it: a repository that exercises an interface in CI already ships a way to drive it, and reaching for that is part of executing the step. What the search covered belongs in the report.
...
- Report per step, never in aggregate, using the verdicts `git-verify-pr` defines: `pass`, `fail`, `blocked (project)` (nothing here can be exercised at all) or `blocked (step)` (this step alone cannot run), each with evidence. The blocked kind is part of the verdict, not a note beside it - it is what tells a standing gap from a one-off.
