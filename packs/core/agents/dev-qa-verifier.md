...
- A step it cannot execute is `blocked` and stays blocked - never a pass because everything around it passed, and never a pass inferred from reading the code. Which kind of blocked belongs in the report: a project with no environment at all is a standing gap the owner closes once, while a step missing a credential belongs to this change alone.
