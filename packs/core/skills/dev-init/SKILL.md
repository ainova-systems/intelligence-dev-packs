...
7. **Line endings - written when absent, reported when present.** Generated output is written LF on every platform, so a checkout that converts text to CRLF puts two endings in one working tree, and every regenerated file reads as a whole-file diff on any machine that is not converting them back. Detect with `git check-attr eol -- <one path per tracked text extension, the generated output among them>`: `unspecified` is the gap, since `core.autocrlf` then decides it per machine. No `.gitattributes` at the repository root - write one carrying `* text=auto eol=lf`, then `git add --renormalize .` so the index and the working tree agree; without that, whatever is already checked out as CRLF stays dirty. One already there - report the gap and that line, and leave the file alone: the last matching pattern wins, so an appended catch-all silently overrides every rule above it, and which of those was deliberate is the owner's call.
