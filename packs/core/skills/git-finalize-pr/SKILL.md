...
**The first pass is round 1, and the budget is read here** - before any of the round's work, so a run that has completed profile `max_pr_rounds` of them begins none, and no round is ever judged after the budget ran out. Both ways back in count: step 5's own fix, and a fix `git-complete-pr` pushed before returning control at step 6. A project that needs more raises the key; a run never grants itself one, the judgement being the thing a budget exists to bound.
