...
- **`ai:manual`** - anything needs the owner: an escalated thread, a `blocked (step)` verification the step itself did not cause, a standing `blocked (project)` gap a non-interactive run could not ask about, a judging stage that could not produce a verdict at all, a stop rule `git-finalize-pr` hit, a decision the task left open. List each item and the decision it needs, and let a `blocked (project)` item name the two profile keys that end it.
