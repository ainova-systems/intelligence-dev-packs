...
2. Pre-flight: source branch current (`git pull --ff-only`), `git status --porcelain` clean, CI green on HEAD. Red - STOP; that is `git-finalize-pr` territory.
...
- Getting branches green and merged - `git-finalize-pr` / `git-merge-pr`.
