...
6. `git commit -m "<the step-5 message>"`, then `git push` (`-u origin <branch>` on first push). The message travels as an argument: a bare `git commit` opens the configured editor and the run stops there until a human closes the window. A body, or any message carrying a backtick or a quote, goes in a file and commits with `-F <file>` - that punctuation is live syntax to bash and to PowerShell alike, which is why `git-open-pr` writes its body to a file too. Remote moved - integrate per profile `update_strategy`, re-run gates, push again. Never force-push.
