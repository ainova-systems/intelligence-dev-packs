...
6. `git commit -m "<the step-5 message>"`, then `git push` (`-u origin <branch>` on first push). The message travels as an argument: a bare `git commit` opens the configured editor and the run stops there until a human closes the window. A body, where the change earns one, is a second `-m` or the multiline quoting of the shell actually running the command. Remote moved - integrate per profile `update_strategy`, re-run gates, push again. Never force-push.
