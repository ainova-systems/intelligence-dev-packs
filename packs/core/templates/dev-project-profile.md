...
- defect_log: auto                  <!-- where a defect found but not fixed in the same change is recorded (dev-context-engineering): auto = the tracker when `tracker` resolves, else ask once | tracker | roadmap | specs (a finding under specs_dir) | <repo-relative file, e.g. docs/known-issues.md> -->
