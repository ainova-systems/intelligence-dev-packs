...
Turn a task into a spec a cold-context session can execute without re-discovering conventions. Ambiguous scope triggers a grill - a relentless, checkpointed interview. The output is the owner's gate 1 review artifact; execution is owned by `dev-execute-spec`.
...
6. **Cross-link.** Plan phases point to feature-doc business rules; requirements point to the feature-doc section. Open business questions stay in the feature doc; the spec tracks only this change's todos. Decisions that crystallised during the grill and pass the ADR gate go to `dev-add-decision`.
...
- Code, branches, PRs - out of scope: `dev-execute-spec` owns execution after approval.
- Decisions made while planning - `dev-add-decision`.
